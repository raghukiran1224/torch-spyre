...
def expand_kv_heads(kv, num_q_heads):
    """Expand KV heads for GQA: [B, KV_H, L, D] -> [B, Q_H, L, D]."""
    bsz, kv_heads, seq_len, head_dim = kv.shape
    if kv_heads == num_q_heads:
        return kv
    n_rep = num_q_heads // kv_heads
    return kv.unsqueeze(2).expand(bsz, kv_heads, n_rep, seq_len, head_dim).reshape(
